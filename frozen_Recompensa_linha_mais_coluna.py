...
learning_rate = 1.0 # taxa de aprendizagem 100% busca o que já aprendeu
...
for episodio in range(num_episodes):
    ...
    print(f'Episódio de número: {episodio}')
    done = False
    ...
    while not done:
        ...
        if next_state_key == state_key:
            reward -= 5 # Punição para evitar que o próximo estado seja o mesmo do anterior e ficar parado 

        if (next_state_key[0] < state_key[0] or next_state_key[1] < state_key[1]):
            reward -= 100   # Uma punião por ir para um estado mais longe do prêmio do que o estado atual, ou seja, evitar de ficar volttando  

        #print(f'Recompensa do Estado{next_state_key}: {reward}')        
